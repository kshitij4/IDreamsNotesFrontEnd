...
const NotesList = () => {
  ...
  const addData = async (obj) => {
    ...
    console.log(obj);
    let result = await axios.post(`http://localhost:8000/notes/userId/${userId}/createnote`, obj,
  ...
  const updateStatus = async (_id, status) => {
    const authToken = localStorage.getItem("token");
    const userId = localStorage.getItem("userId");
    let result = await axios.post(`http://localhost:8000/notes/userId/${userId}/noteId/${_id}/updateNoteStatus`, { status },
      { headers: { authorization: `Bearer ${authToken}` } });
    if (result.data.isSuccess) {
      setChange(prev => !prev);
    } 
  ...
  async function getData() {
    ...
    const authToken = localStorage.getItem("token");
    const userId = localStorage.getItem("userId");
    let result = await axios.get(`http://localhost:8000/notes/userId/${userId}/getAllNotes`,
      { headers: { authorization: `Bearer ${authToken}` } });
    console.log(result);
    if (result.data.isSuccess) {
